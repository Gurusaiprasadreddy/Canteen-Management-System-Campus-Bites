...
const EMPTY = { id: '', name: '', location: '', operatingHours: '', status: 'Open' };

export default function CanteenMasterPage() {
    ...
    const load = () => axios.get(`${API}/canteens`).then(r => setRows(r.data)).catch(() => { });
    useEffect(() => { load(); }, []);

    const handleInsert = async () => {
        try { await axios.post(`${API}/canteens`, form); notify('✅ Canteen inserted!'); setForm(EMPTY); load(); }
        catch (e) { notify('❌ ' + (e.response?.data?.detail || e.message)); }
    };
    const handleUpdate = async () => {
        try { await axios.put(`${API}/canteens/${form.id}`, form); notify('✅ Canteen updated!'); setForm(EMPTY); setEditMode(false); load(); }
        catch (e) { notify('❌ ' + (e.response?.data?.detail || e.message)); }
    };
    const handleDelete = async (id) => {
        if (!window.confirm(`Delete canteen ${id}?`)) return;
        try { await axios.delete(`${API}/canteens/${id}`); notify('✅ Deleted!'); load(); }
        catch (e) { notify('❌ ' + (e.response?.data?.detail || e.message)); }
    };
    const handleSearch = async () => {
        try { const r = await axios.get(`${API}/canteens/${searchId}`); setSearchResult(r.data); }
    ...
    return (
        <div style={styles.page}>
            <h2 style={styles.title}>🏪 M3 — Canteen Master</h2>
            {msg && <div style={styles.msg}>{msg}</div>}
            <div style={styles.card}>
                <h3 style={styles.cardTitle}>{editMode ? '✏️ Update Canteen' : '➕ Insert Canteen'}</h3>
                <div style={styles.grid}>
                    {[['id', 'Canteen ID'], ['name', 'Canteen Name'], ['location', 'Location'], ['operatingHours', 'Operating Hours']].map(([k, lbl]) => (
            ...
            <div style={styles.card}>
                <h3 style={styles.cardTitle}>🔍 Search by Canteen ID</h3>
                <div style={{ display: 'flex', gap: 8 }}>
                    <input style={{ ...styles.input, flex: 1 }} value={searchId} onChange={e => setSearchId(e.target.value)} placeholder="Enter Canteen ID" />
            ...
            <div style={styles.card}>
                <h3 style={styles.cardTitle}>📋 Display All Canteens</h3>
                {rows.length === 0 ? <p style={{ color: '#888' }}>No records found.</p> : (
                    <div style={{ overflowX: 'auto' }}>
                        <table style={styles.table}>
                            <thead><tr><th style={styles.th}>ID</th><th style={styles.th}>Name</th><th style={styles.th}>Location</th><th style={styles.th}>Hours</th><th style={styles.th}>Status</th><th style={styles.th}>Actions</th></tr></thead>
                            <tbody>{rows.map(row => (
                                <tr key={row.id}>
                                    <td style={styles.td}>{row.id}</td><td style={styles.td}>{row.name}</td><td style={styles.td}>{row.location}</td>
                                    <td style={styles.td}>{row.operatingHours}</td><td style={styles.td}>{row.status}</td>
                                    <td style={styles.td}><button style={styles.btnEdit} onClick={() => startEdit(row)}>Edit</button><button style={styles.btnDel} onClick={() => handleDelete(row.id)}>Delete</button></td>
                                </tr>))}</tbody>
                        </table>
                    </div>
                )}
