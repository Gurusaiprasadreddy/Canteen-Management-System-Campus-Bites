...
async def submit_rating(
    ...
    """Submit a star rating (1-5) for a menu item. One rating per user per item."""
    if stars < 1 or stars > 5:
        raise HTTPException(status_code=400, detail="Stars must be between 1 and 5")

    # Check item exists
    item = await db.menu_items.find_one({"item_id": item_id})
    if not item:
    ...
    await db.ratings.update_one(
        ...
        {"$set": {
            ...
            "item_id": item_id,
            "canteen_id": item.get("canteen_id", ""),
            "stars": stars,
            "review": review,
            "updated_at": now
    ...
    # Recalculate average rating for this item and save back to menu_items
    pipeline = [
        {"$match": {"item_id": item_id}},
        {"$group": {"_id": "$item_id", "avg": {"$avg": "$stars"}, "count": {"$sum": 1}}}
    ]
    ...
    return {"message": "Rating submitted!", "stars": stars, "avg_rating": avg if result else stars}


@api_router.get("/ratings/{item_id}")
async def get_item_ratings(item_id: str):
    """Get all ratings for a specific menu item (public, no auth needed)."""
    ratings = await db.ratings.find({"item_id": item_id}, {"_id": 0}).to_list(200)
    ...
    rating_count = agg[0]["count"] if agg else 0
